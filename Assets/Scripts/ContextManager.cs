    ...
    public void ExecutePreviousInstructions(Instruction instruction)
    {
        ...
        foreach (var previousInstruction in previousInstructions)
        {
            ...
            foreach (var action in previousInstruction.Actions)
            {
                ...
                var ifmAttachingObj =
                    Helpers.FindObjectInFigure(
                        taskType == Constants.TaskType.Installation ? Constants.FigureType.Ifm : Constants.FigureType.Reference,
                        action.Components[0]);
                var ifmReferenceObj =
                    Helpers.FindObjectInFigure(
                        taskType == Constants.TaskType.Installation ? Constants.FigureType.Ifm : Constants.FigureType.Reference, "core");
